...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        if user_created:
            ...
            user.save()
            self.stdout.write(
                self.style.WARNING("Пользователь moderator@example.com уже существует. Пароль обновлен."))

        # Добавляем пользователя в группу модераторов
        user.groups.add(group)
        self.stdout.write(self.style.SUCCESS("Пользователь добавлен в группу 'Модератор продуктов'"))
